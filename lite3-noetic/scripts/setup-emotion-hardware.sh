...
ssh "${SSH_OPTIONS[@]}" "${JUMP_OPTIONS[@]}" "${PERCEPTION_SSH}" \
  "set -e; for attempt in \$(seq 1 20); do if test \"\$(systemctl is-active 'emotion-bot-hardware-core@${ROBOT_USER}.service')\" = active && test \"\$(systemctl is-active 'emotion-bot-hardware-telemetry@${ROBOT_USER}.service')\" = active; then break; fi; sleep 0.25; done; test \"\$(systemctl is-active 'emotion-bot-hardware-core@${ROBOT_USER}.service')\" = active; test \"\$(systemctl is-active 'emotion-bot-hardware-telemetry@${ROBOT_USER}.service')\" = active; source /opt/ros/noetic/setup.bash; source ~/emotion_bot_lite3_hw_ws/devel/setup.bash; test \"\$(rosparam get /emotion_bot/hardware/safety/transmit_enabled)\" = false; test \"\$(rosparam get /emotion_bot/hardware/safety/dynamic_actions_enabled)\" = false; timeout 8 rostopic echo -n 1 /emotion_bot/hardware/expression_status; timeout 8 rostopic echo -n 1 /emotion_bot/hardware/telemetry_health"
