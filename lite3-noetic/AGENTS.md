...
- Neutral is the only checked-in normal commissioned category. The accepted 50 mm alternating front-paw joy trajectory currently runs only through the explicit bounded joy suite; its normal chat selection and mid-lift retargeting are unfinished. Anger's contact-gated normal selection and safe retarget path are implemented and tested offline. After the original repeat and first x/y-only revision failed a repeated right-side support gate, the canonical-reset revision passed all seven aarch64 suites and its explicit three-cycle physical run. Normal chat selection and retargeting remain live-unvalidated, so the allowlist stays `neutral`. Treat the other six final profiles as planned and the older planted profiles as prototypes/fallbacks.
